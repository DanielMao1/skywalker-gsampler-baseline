...
template <typename T>
struct buffer_group<T, thread_block_tile<32>, BufferType::GMEM,
                    AliasTableStorePolicy::NONE> {
  uint size;
  float weight_sum;
  T *ids;
...
struct alias_table_constructor_shmem<T, thread_block_tile<32>,
  ...
  buffer_group<T, thread_block_tile<32>, BufferType::GMEM> buffer;
  ...
  __device__ void Clean() {
    // if (LID == 0) {
    buffer.large.CleanWC();
    buffer.small.CleanWC();
    buffer.alias.CleanWC();
    buffer.prob.CleanWC();
    buffer.selected.CleanWC();
    // }
  }
  template <typename sample_result>
  __device__ void walk(T *array, curandState *state, sample_result result) {
    ...
    while (!buffer.small.Empty() && !buffer.large.Empty()) {
#ifdef SPEC_EXE
      {
        ...
        if (old_small_idx >= 0) {
          ...
          if (active.thread_rank() == 0) {
            // buffer.small.size -= MIN(buffer.small.Size(), active.size());
            buffer.small.SizeAtomicAdd(
                -MIN(buffer.small.Size(), active.size()));
          }
